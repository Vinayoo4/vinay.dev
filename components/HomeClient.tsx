...
import JourneySection from "@/app/journey/page";
...
function SectionPreview(props: { title: string; description: string; href: string; icon: React.ElementType }) {
  const { title, description, href, icon: Icon } = props;
  return (
    <div className="rounded-xl bg-yellow-500/5 border border-yellow-500/10 p-6 flex flex-col items-start mb-8">
      <div className="flex items-center mb-2">
        <Icon className="w-6 h-6 text-yellow-500 mr-2" />
        <h2 className="text-2xl font-bold text-yellow-500">{title}</h2>
      </div>
      <p className="text-gray-400 mb-4">{description}</p>
      <Link href={href} className="inline-block px-4 py-2 rounded bg-yellow-500 text-black font-semibold hover:bg-yellow-400 transition-colors">Read more</Link>
    </div>
  );
}
